...
clave_css = '''
/* Clave Catastral Segmented */
.atlas-clave-container {
  display: flex;
  gap: 4px;
  align-items: center;
  position: relative;
...
if '.atlas-clave-container' not in css:
    css += clave_css
    with open(css_path, 'w', encoding='utf-8') as f:
        f.write(css)
    print("Updated ShapefileAtlasModal.css with segmented clave styles.")

# 2. Update JSX with Segmented Clave Catastral
new_jsx = '''import React, { useState, useEffect, useMemo, useRef, useContext } from 'react';
import { 
...
export default function ShapefileAtlasModal({ 
  ...
  // Auto-llenado de Provincia y Cantón según Empresa activa
  useEffect(() => {
    if (activeEmpresa) {
      const autoFillDPA = async () => {
        try {
          ...
          if (provMatch) {
            let prefix = provMatch.codigo_dpa;
            ...
            if (cantMatch && cantMatch.codigo_dpa) {
              prefix += cantMatch.codigo_dpa.substring(2, 4);
            }
            setDpaPrefix(prefix);
          }
        } catch (e) {
  ...
  const normalizedFeatures = useMemo(() => {
    ...
    geoJsonData.features.forEach((feat, idx) => {
      ...
      rawRings.forEach((ring, ringIdx) => {
        ...
        // Centroide para posicionar etiquetas
        let latSum = 0;
        let lngSum = 0;
        const nPoints = latLngs.length - 1;
  ...
  useEffect(() => {
    ...
    normalizedFeatures.forEach((feat, idx) => {
      ...
      let initCedula = '';
      let initNombre = '';
      let initClave = dpaPrefix ? dpaPrefix.padEnd(19, ' ') : '';
...
with open(jsx_path, 'w', encoding='utf-8') as f:
    f.write(new_jsx)
print("Updated ShapefileAtlasModal.jsx with segmented Clave Catastral.")
